 ...
 */
@WebServlet("/SellerIconUpload")
@MultipartConfig()
public class SellerIconUpload extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public SellerIconUpload() {
        super();
    }
	...
	private void processRequest(HttpServletRequest request,
		...
		String ext = null;
		String dir=getServletContext().getRealPath("/upload/SellerIcon");
		String picName = StringHandler.createSerialId(System.currentTimeMillis());
		for (FileItem item : items) {
			if (item.isFormField()) {
				//此时这个item项中的内容是普通的文本
				sellerId=item.getString();//取得参数内容
			} else {
				//这是对非文本内容的处理
		...
		Seller seller = sellerDaoImpl.findAll("where s_id='"+ sellerId +"'").get(0);
		seller.setSeller_icon("/upload/SellerIcon/" + picName + ext);
