...
/**
 * Servlet implementation class RestIconUpload
 */
@WebServlet("/RestDishIconUpload")
@MultipartConfig()
public class RestDishIconUpload extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public RestDishIconUpload() {
        super();
    }
	...
	private void processRequest(HttpServletRequest request,
		...
		String rdId = null;
		String ext = null;
		String dir=getServletContext().getRealPath("/upload/RestDishIcon");
		String picName = StringHandler.createSerialId(System.currentTimeMillis());
		for (FileItem item : items) {
			if (item.isFormField()) {
				//此时这个item项中的内容是普通的文本
				rdId=item.getString();//取得参数内容
			} else {
				//这是对非文本内容的处理
		...
		RestDish rd = restDishDaoImpl.findAll("where rd_id='"+ rdId +"'").get(0);
		rd.setRd_icon("/upload/RestDishIcon/" + picName + ext);
