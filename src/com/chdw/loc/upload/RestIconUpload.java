 ...
 */
@WebServlet("/RestIconUpload")
@MultipartConfig()
public class RestIconUpload extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public RestIconUpload() {
        super();
    }
	...
	private void processRequest(HttpServletRequest request,
		...
		String restId = null;
		String ext = null;
		String dir=getServletContext().getRealPath("/upload/RestIcon");
		String picName = StringHandler.createSerialId(System.currentTimeMillis());
		for (FileItem item : items) {
			if (item.isFormField()) {
				//此时这个item项中的内容是普通的文本
				restId=item.getString();//取得参数内容
			} else {
				//这是对非文本内容的处理
		...
		Restaurant rest = restaurantDaoImpl.findAll("where r_id='"+ restId +"'").get(0);
		rest.setR_icon("/upload/RestIcon/" + picName + ext);
