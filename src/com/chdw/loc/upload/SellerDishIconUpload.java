...
/**
 * Servlet implementation class SellerIconUpload
 */
@WebServlet("/SellerDishIconUpload")
@MultipartConfig()
public class SellerDishIconUpload extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public SellerDishIconUpload() {
        super();
    }
	...
	private void processRequest(HttpServletRequest request,
		...
		String sdId = null;
		String ext = null;
		String dir=getServletContext().getRealPath("/upload/SellerDishIcon");
		String picName = StringHandler.createSerialId(System.currentTimeMillis());
		for (FileItem item : items) {
			if (item.isFormField()) {
				//此时这个item项中的内容是普通的文本
				sdId=item.getString();//取得参数内容
			} else {
				//这是对非文本内容的处理
		...
		SellerDish sd = sellerDishDaoImpl.findAll("where sd_id='"+ sdId +"'").get(0);
		sd.setSd_icon("/upload/SellerDishIcon/" + picName + ext);
